...
def carregar_dados():
    try:
        ...
        # Garante que .unique() sempre retorne um array vazio se a coluna não existir ou for toda NaN
        # E que a conversão para lista seja robusta
        secretarias_unicas = sorted(list(df['Secretaria'].dropna().unique())) if 'Secretaria' in df.columns else []
        tipos_unicas = sorted(list(df['Tipo'].dropna().unique())) if 'Tipo' in df.columns else []
        subtipos_unicas = sorted(list(df['Subtipo'].dropna().unique())) if 'Subtipo' in df.columns else []
        projetos_unicos = sorted(list(df['nome'].dropna().unique())) if 'nome' in df.columns else []
        situacoes_unicas = sorted(list(df['Status do Projeto'].dropna().unique())) if 'Status do Projeto' in df.columns else []

        return df, secretarias_unicas, tipos_unicas, subtipos_unicas, projetos_unicos, situacoes_unicas
...
df, secretarias_unicas, tipos_unicas, subtipos_unicas, projetos_unicos, situacoes_unicas = carregar_dados()

# --- DEBUG: Verifique o estado das variáveis antes dos multiselects ---
st.write(f"df.empty: {df.empty}")
st.write(f"secretarias_unicas: {secretarias_unicas}")
st.write(f"tipos_unicas: {tipos_unicas}")
st.write(f"subtipos_unicas: {subtipos_unicas}")
st.write(f"projetos_unicos: {projetos_unicos}")
st.write(f"situacoes_unicas: {situacoes_unicas}")
# --- FIM DEBUG ---
